...
winui.onDOMContentLoaded(function(){
	...
	WINUI_MENUBAR_MENUTABLE_STYLE = "z-index:999999;border:2px;border-style:outset;border-color:#eee;padding:0px;padding-bottom:1px;margin:0px;width:150px;position:absolute;background-color:#D4D0C8;display:none;";		//默认样式
	WINUI_MENUBAR_MENUITEM_DEFAULT_STYLE = "color:#000;background-color:#D4D0C8;width:132px;height:18px;line-height:18px;text-align:left;padding-left:17px;overflow:hidden;";	//一级以下菜单项默认样式
	WINUI_MENUBAR_MENUITEM_MOUSEOVER_STYLE = "color:#fff;background-color:#0A246A;width:132px;height:18px;line-height:18px;text-align:left;padding-left:17px;overflow:hidden;";	//当鼠标移动至菜单后样式
	
	/*多级菜单表样式*/
	var WINUI_MENUBAR_MULTISTAGE_MENUTABLE_STYLE = "";				//菜单表默认样式
	
	WINUI_MENUBAR_MULTISTAGE_MENUTABLE_STYLE = "z-index:999999;border:2px;border-style:outset;border-color:#eee;padding:0px;margin-top:-18px;margin-left:146px;width:150px;position:absolute;background-color:#D4D0C8;display:none;";		//默认样式
	...
	var menuReset = function(event){
        ...
        //如果是被禁用的菜单项，则退出不做任何动作
        if(evtsrc.getAttribute("clickEvent")!=null && evtsrc.getAttribute("clickEvent")!=""){
        	return;
        }
	...
	function menuTableProcess(mt){
		...
		if(mt.parentNode.parentNode.getAttribute("type")!="menubar"){
			...
			/*
			if(refrence.getAttribute("clickEvent")!=null){
				refrence.style.color="#aaaaaa";
			}
			*/
			...
			refrence.onmouseover=function(event){
				...
				//if(div==refrence.parentNode){
					...
					if(div.getAttribute("clickEvent")!=null && div.getAttribute("clickEvent")!=""){
			...
			refrence.onmouseout=function(event){
		...
		for(var i=0;i<nodes.length;i++){
			...
			//如果是菜单项
				...
				/*
				if(nodes[i].getAttribute("clickEvent")!=null){
					nodes[i].style.color="#aaaaaa";
				}
				*/
				//当鼠标移动到菜单项时
				nodes[i].onmouseover=function(event){
					...
					if(currentDisplaiedMenuTable!=null && div.parentNode!=currentDisplaiedMenuTable){
						currentDisplaiedMenuTable.style.display="none";
...
//根据菜单名称启用或禁用菜单
winui.setMenuEnabled=function(menuText,flag){
	
	var menubar = winui.getElements("DIV","type","menubar")[0];
	
	var menus = menubar.getElementsByTagName("DIV");
	
	for(var i=0;i<menus.length;i++){
		var menu = menus[i];
		if(menu.innerHTML==menuText){
			
			if(flag){

				menu.style.color="#000000";
				menu.setAttribute("onclick",menu.getAttribute("clickEvent"));
				menu.removeAttribute("clickEvent");
				
				
			}else{

				menu.style.color="#aaaaaa";
				var click = menu.getAttribute("onclick");
				if(click==null || click==""){
					click="void(0)";
				}
				menu.setAttribute("clickEvent",click);
				menu.onclick=null;//IE9下 必须这样才有效
...
//根据菜单名称数组启用或禁用菜单
winui.setMenuEnabledByArray=function(menuTextArray,flag){
	...
	for(var i=0;i<menus.length;i++){
		...
		for(var j=0;j<menuTextArray.length;j++){
			
			
			if(menu.innerHTML==menuTextArray[j]){
				if(flag){
				...
				menuTextArray.splice(j,1);//删除已经找到的元素
...
//设置菜单项显示方式，可选值：none、block、""
winui.setMenuDisplay=function(menuText,display){
...
//设置菜单数组显示和隐藏菜单项，可选值：none、block、""
winui.setMenuDisplayByArray=function(menuTextArray,display){
